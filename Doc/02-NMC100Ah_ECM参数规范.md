...
| 与特性文档关系 | 本规范把特性文档中的定性规律落成可计算的默认函数；§11 对应 `Doc/01` §7 |
...
> 特性：`Doc/01` §7。本节只给**模板估算**，供仿真生成器选用。  
...
叠加会把一部分扩散**算两次**（1RC 的 \(R_1\) 里已经吞了一点慢过程）。这是有意的：BMS 一阶去拟合这样的电压，就会留下 §11.5 和 `Doc/04` §10 要的慢残差。真电芯重标请用拆分，不要把叠加参考值写进控制器。
...
1C 下 6–24 mV 的慢分量，若被 EKF 整段啃进 SOC：中段 \(\partial U_{\mathrm{ocv}}/\partial s\sim 0.4\,\mathrm{V}\) 时约 1.5–6 个百分点的短暂偏差，回弹结束后应退回。定量误差预算见 `Doc/04` §10。
