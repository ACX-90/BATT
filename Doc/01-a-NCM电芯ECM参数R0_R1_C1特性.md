...
- **19%**：bulk 已经在膝部且在**加速**（\(dU/ds\) 前半 0.94 V → 后半 1.53 V），\(\eta\) 还不饱和（前后半斜率比 1.38）。OCV 后半变陡、\(\eta\) 后半略慢，对消成 \(U_t\) 前后半 −0.80 / −0.81 mV/s。电流仍是 100 A（`--edge-c-half` 要 \(s<0.15\) 才削放电，那是 `s09` 的 0.3C，不要和 19% 混）。
