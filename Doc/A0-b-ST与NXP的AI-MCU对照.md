...
- 结论：当 BCU 主核**成立**；当「AI MCU」**不对等**。03-d ifx_demo 能转，A0-a 的优化清单（KF、电流闸、降频）一样要补。
...
输入仿射仍按 `Doc/03-d` §1.1。NPU 量化图不能直接吃本仓库 `scaler.json` 的 z-score，要在预训练里就定成 ifx_demo 那套或单独标定。
...
1. **03-d ifx_demo 在三家都能眨眼。** 50 个数、180 个头不是选型依据。   
