...
> **说明**：1RC 是工程折中。电化学上至少存在“电荷转移（快）+ 固/液相扩散（慢）”两个时间尺度。若精度不够，应升到 2RC（\(R_{1}C_{1}\) 快极化 + \(R_{2}C_{2}\) 慢扩散）。下文默认 1RC，并在必要时注明 2RC 的对应关系。

EIS 对照（便于理解，不要求在线测）：

- \(R_{0}\) ≈ Nyquist 图高频实轴截距（\(R_{\Omega}\)）
- \(R_{1}\) ≈ 中频半圆直径（\(R_{\mathrm{ct}}+R_{\mathrm{SEI}}\) 的集总）
- \(C_{1}\) ≈ 半圆对应的等效电容；扩散尾巴（Warburg）在 1RC 中被部分并入 \(R_{1}C_{1}\)

HPPC / 脉冲对照：

- \(R_{0} = \Delta U_{\mathrm{instant}} / I\)（电流阶跃后第一个采样点的电压跳变）
- \(R_{1} \approx (\Delta U_{\mathrm{end}} - \Delta U_{\mathrm{instant}}) / I\)（脉冲末期极化饱和压降）
- \(C_{1} = \tau_{1} / R_{1}\)，\(\tau_{1}\) 由搁置回弹或脉冲上升沿指数拟合得到

---

## 2. 三个参数的共性

NCM 电芯的 \(R_{0}\)、\(R_{1}\)、\(C_{1}\) **都不是常数**，工程上必须建成多维表或函数：

\[
P = f(\mathrm{SOC},\; T,\; I,\; \mathrm{方向},\; \mathrm{SOH},\; \mathrm{体系})
\]

其中 \(P \in \{R_{0}, R_{1}, C_{1}\}\)。各因子的相对强度大致为：

| 影响因素 | \(R_{0}\) | \(R_{1}\) | \(C_{1}\) |
|----------|-----------|-----------|-----------|
| 温度 \(T\) | 强（Arrhenius） | 很强（Arrhenius，\(E_{a}\) 更大） | 中等偏弱 |
| SOC | 中（两端升高） | 强（两端升高更明显） | 中（规律不如电阻稳定） |
| 电流倍率 / 方向 | 弱–中 | 中–强（Butler–Volmer 非线性） | 弱 |
| SOH / 老化 | 强、单调升 | 强、总体升 | 总体降 |
| 充/放不对称 | 弱–中 | 中 | 弱 |
| 辨识可观性 | 高、稳健 | 中 | 偏低，易与 \(R_{1}\) 耦合 |
...
| 功能 | 主要敏感参数 | NCM 上要注意 |
|------|----------------|--------------|
| SOC（EKF/AEKF 等） | OCV 为主，\(R_{0}/R_{1}\) 影响动态新息 | 两端 \(R_{1}\) 失配会导致 SOC 在满/空附近拉偏 |
| SOP | \(R_{0}+R_{1}(1-e^{-\Delta t/\tau_{1}})\) | 必须带 T、SOC，低温加电流非线性 |
| SOH | \(R_{0}\) 首选，\(R_{1}\) 辅助 | 高镍快充后 \(R_{0}\) 比容量掉得更早 |
| 热模型 | \(I^{2}R_{0} + I\cdot U_{p}\) | 低温 \(R_{1}\) 产热占比上升 |
...
- 1 s 功率：接近 \(R_{0}\)
- 10 s 功率：\(R_{0}+0.5R_{1}\) 量级（取决于 \(\tau_{1}\)）
- 持续功率：逼近 \(R_{0}+R_{1}\)（还要叠加 OCV 随 SOC 下滑）
...
\frac{\partial C_{1}}{\partial T}\gtrsim 0 \\[4pt]
R_{0},R_{1}\ \text{在低/高 SOC 升高},\quad
R_{0},R_{1}\ \text{随 SOH 下降而升高},\quad
C_{1}\ \text{随 SOH 下降而降低}
\end{aligned}
\]
...
*文档性质：NCM 一阶 ECM 参数特性说明，供标定、建模与 BMS 使用。具体数值必须用目标电芯的 HPPC/EIS/工况数据重新辨识，上表量级不可直接当标称值写入控制器。*
)
