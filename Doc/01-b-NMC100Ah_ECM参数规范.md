...
这不是全量 Butler–Volmer。0.1 s 工程 \(R_0\) 已含快电荷转移，多倍率边沿要对齐时用 `Doc/01-a` §3.4 的 \(h_{\mathrm{BV}}\) 或 \(\eta_{\mathrm{BV}}\)；不要和下面 \(R_1\) 的 asinh 各乘一份。
...
NMC 的 OCV 有斜率，长脉冲必须从 \(\Delta U\) 中减去 \(\Delta U_{\mathrm{ocv}}\)，否则 \(R_1\)（以及 \(R_2\)）被系统性夸大。扣的是平均计量的 bulk OCV。两端 SOC（约 90%、约 20%）上，扣完之后恒流段 \(\eta\) 仍可能近乎直线——那是颗粒表面 OCP 的非线性，不是又一只 RC，也不是 PNGV 电容；\(\tau\) 仍用搁置回弹，见 `Doc/01-a` §8.6。
