...
def _nlinear_c_table(table: np.ndarray, log_r: bool = False) -> str:
    vals = np.log(np.clip(table, 1e-12, None)) if log_r else table
    ...
    lines.append("{")
    for i in range(rows):
        row_strs = [fmt.format(vals[i, j]) for j in range(cols)]
...
def _nlinear_c_3d(table: np.ndarray, log_r: bool = False) -> str:
    vals = np.log(np.clip(table, 1e-12, None)) if log_r else table
    n_i, n_s, n_t, n_ch = vals.shape
    fmt = "%.6ff"
    lines: list[str] = []
    lines.append("{")
    for ii in range(n_i):
        lines.append(f"    /* i_ax[{ii}] */")
        for ss in range(n_s):
            lines.append(f"    /* s_ax[{ss}] */")
            for tt in range(n_t):
                r0 = fmt.format(vals[ii, ss, tt, 0])
                r1 = fmt.format(vals[ii, ss, tt, 1])
                comma = "," if (ii, ss, tt, _) != (n_i - 1, n_s - 1, n_t - 1, 1) else ""
                lines.append(f"      {{{r0}, {r1}}}{comma}")
    lines.append("}")
    return "\n".join(lines)


def _axes_c_array(ax: np.ndarray, name: str, dim_name: str, size_name: str) -> str:
    fmt = "%.6ff"
    values = ", ".join(fmt.format(v) for v in ax)
    return (
        f"#define LUT_{size_name} {len(ax)}\n"
        f"static const float {name}[LUT_{size_name}] = {{ {values} }};\n"
    )
...
def _write_c_header(
    ...
    ax_names = ["i_ax", "s_ax", "t_ax"]
    dim_names = ["N_I", "N_S", "N_T"]
    lines: list[str] = []
    lines.append(f"/* auto-generated by bake_lut.py - do not edit */")
    lines.append(f"/* LUT: {kind} {name_suffix} */")
    lines.append(f"/* Generated at: {np.datetime64('now')} */")
    lines.append("")

    for d, (ax, aname, dname) in enumerate(zip(axes, ax_names, dim_names)):
        lines.append(_axes_c_array(ax, aname, aname.lower(), dname))
        lines.append("")

    n_ch = table.shape[-1] if table.ndim > 1 else 2
    lines.append(f"#define LUT_N_CH {n_ch}")
    lines.append(f"#define LUT_LOG_R {'true' if log_r else 'false'}")
    lines.append("")

    if kind == "3d":
        lines.append("/* R0/R1 lookup table [N_I][N_S][N_T][N_CH] */")
        lines.append("static const float lut_table[LUT_N_I][LUT_N_S][LUT_N_T][LUT_N_CH] = ")
        lines.append(_nlinear_c_3d(table, log_r))
        lines.append("")
        total_nodes = len(axes[0]) * len(axes[1]) * len(axes[2])
    elif kind == "2d_fixed":
        lines.append("/* R0/R1 lookup table [N_S][N_T][N_CH] */")
        lines.append("static const float lut_table[LUT_N_S][LUT_N_T][LUT_N_CH] = ")
        lines.append(_nlinear_c_table(table, log_r))
        lines.append("")
        total_nodes = len(axes[0]) * len(axes[1])
    else:
        lines.append(f"/* unsupported kind: {kind} */")
        lines.append("")
        total_nodes = 0

    rom_bytes_total = total_nodes * n_ch * 4
    lines.append(f"/* ROM footprint: {rom_bytes_total} bytes ({rom_bytes_total / 1024:.1f} KB) */")
    lines.append(f"#define LUT_ROM_BYTES {rom_bytes_total}")
    lines.append("")

    lines.append("/* Configuration */")
    lines.append(f"/* axes_box: {json.dumps(config_info.get('axes_box', {}))} */")
    lines.append(f"/* teacher_dir: {config_info.get('teacher_dir', '')} */")
    lines.append(f"/* lut_mode: {kind} */")
    lines.append(f"/* log_r: {log_r} */")
    lines.append("")

    header_path = out_dir / f"lut_{name_suffix}.h"
...
def _write_c_binary(
    ...
    flat = table.astype(np.float32).ravel()
    bin_path = out_dir / f"lut_{name_suffix}.bin"
...
def main() -> None:
    ...
    # New args for build_lut integration and C export
    p.add_argument("--mode", choices=["3d", "2d_fixed", "sep"], default=None)
    p.add_argument("--n-i", type=int, default=None)
    p.add_argument("--n-s", type=int, default=None)
    p.add_argument("--n-t", type=int, default=None)
    p.add_argument("--export-c", action="store_true", help="Export C header + binary")
    p.add_argument("--lut-mode", choices=["3d", "2d_fixed", "sep"], default=None, dest="custom_mode")
    p.add_argument("--custom-name", default=None, help="Custom name suffix for output files")
    args = p.parse_args()
    ...
    # Custom single config from build_lut.bat args
    if any(v is not None for v in [args.mode, args.n_i, args.n_s, args.n_t, args.export_c]):
        kind = args.custom_mode or args.mode or "3d"
        log_r = bool(args.log_r)
        n_i = int(args.n_i) if args.n_i is not None else 5
        n_s = int(args.n_s) if args.n_s is not None else 21
        n_t = int(args.n_t) if args.n_t is not None else 13
        name_suffix = args.custom_name or f"{kind}_{n_i}x{n_s}x{n_t}" + ("_log" if log_r else "")
        
        axes, table = bake_3d(model, scaler, n_i, n_s, n_t) if kind == "3d" else (None, None)
        if axes is None:
            print(f"ERROR: kind={kind} not yet supported for custom mode", flush=True)
            return
        ...
        row = {
            ...
            "log_r": log_r,
            "n_node": n_i * n_s * n_t,
            "rom_B": rom_bytes(n_i * n_s * n_t),
            "rom_kb": rom_bytes(n_i * n_s * n_t) / 1024.0,
        }
        results.append(row)
        print(fmt_row(row), flush=True)
        
        np.savez(
            out_dir / f"{name_suffix}.npz",
            i_ax=np.asarray(axes[0]),
            s_ax=np.asarray(axes[1]),
            t_ax=np.asarray(axes[2]),
            table=table.astype(np.float32),
            log_r=np.array(log_r),
        )
        print(f"  wrote {out_dir / f'{name_suffix}.npz'}")
        ...
        if args.export_c:
            ...
            _write_c_header(out_dir, kind, name_suffix, axes, table, log_r, config_info)
            _write_c_binary(out_dir, name_suffix, table.astype(np.float32))
        
        print(f"\nCustom LUT: {name_suffix} ({kind}) saved", flush=True)
