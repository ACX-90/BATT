...
| [`Src/AI/EV_Local/`](Src/AI/KF/readme.md) | 车上滑窗：全局 \(k\)、\(k\) 网格；3×8×2 对照已丢掉（`Doc/05-d`） |
...
指令序列、噪声、初值改 `nmc100ah_gen.py` 文件头部。`--show` 弹窗，`--no-noise` 关噪声。
...
| `nmc100ah_gen_grid.py` | 默认 5×5 起始 SOC × 温度，写 `Data/grid/`；`--pybamm --out-dir Data/grid_pybamm` |
| `nmc100ah_gen_long.py` | 小时级负例；默认 ECM，`--pybamm` 写 `Data/long_pybamm/` |
| `nmc100ah_pybamm.py` | PyBaMM SPM 后端，由上面三个入口的 `--pybamm` 调用 |
...
| 文件 | 做什么 | 实际命令 |
|------|--------|----------|
| [`Script/gen_common.bat`](Script/gen_common.bat) | 单条对照工况，写出 `Data/nmc100ah_ecm_sim.csv` | `python Src/Sim/nmc100ah_gen.py` |
...
| [`Script/kf_neg.bat`](Script/kf_neg.bat) | 小时级负例 + 门控 | `nmc100ah_gen_long.py` 再 `run.py` |
...
`Script/gen_grid.bat` 正式跑之前会清掉 `Data/grid/` 里旧 CSV。工况序列和噪声改 `Src/Sim/nmc100ah_gen.py` 头部，网格和单条仿真共用。`Script/train_1000_resume.bat` 的 `--epochs` 是**再跑多少轮**，不是训到第几轮。`Script/test.bat` 默认用最新 `epoch_XXXXX.pt`；`--best` 改用验证集最好的那份。
...
| [`Doc/04-b-增量学习应用手册.md`](Doc/04-b-增量学习应用手册.md) | 增量怎么用：实验室选档 + 车上 1a / 1b 菜单（头已丢掉） |
| [`Doc/04-c-第0期实验汇报.md`](Doc/04-c-第0期实验汇报.md) | 第 0 期结题汇报（可分享）：完成度、八组读法、带走的规则 |
| [`Doc/05-a-车上增量约束评估.md`](Doc/05-a-车上增量约束评估.md) | 车上增量：无 OTA、数据不外发、单 MCU 资源信封；第 1 期按此收口 |
...
| [`Doc/05-d-车上增量精度评估.md`](Doc/05-d-车上增量精度评估.md) | 第 1 期精度数字；默认写 \(k\) 网格，3×8×2 丢掉 |
| [`Doc/05-e-第1期实验汇报.md`](Doc/05-e-第1期实验汇报.md) | 第 1 期结题汇报（可分享）：1a / 1b / 1c、选档、带走的规则 |
| [`Doc/05-f-车上激励与采样同步暂存.md`](Doc/05-f-车上激励与采样同步暂存.md) | 暂存：20 A 边沿、主动加热脉冲、\(I\)–\(U\) 差几毫秒；§6 下线局部 \(k\) 铺全网格初值（评估，默认仍 \(k=1\)） |
| [`Doc/06-a-第2期包级对照实验方案.md`](Doc/06-a-第2期包级对照实验方案.md) | 第 2 期方案：180 串共用电流、包级 \(b_I\) 门、2A–2I |
| [`Doc/06-b-查表支线与AI支线.md`](Doc/06-b-查表支线与AI支线.md) | 当前实验两条支线：查表当能上车的 SoX；AI 加速器另开对照。§2.2：规格书对 2RC 真数据仍成立，慢尾巴不写 \(k\) |
| [`Doc/06-c-1RC查表残差与策略信封.md`](Doc/06-c-1RC查表残差与策略信封.md) | 2β 接法 Δ、策略信封；用 AI 写 \(k\) 评掉；影子 \(k\)；升 2RC 后仍只写 \(k\) |
| [`Doc/06-d-第2期包级对照实验.md`](Doc/06-d-第2期包级对照实验.md) | 第 2 期数字：2A1–2A4 / 2B / 2E / 2D / 2C / 2G 已跑；下一件 2H |
